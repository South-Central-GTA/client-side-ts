...
export class MarkerSyncModule {
                ...
                if (marker.onDisplay) {
                    ...
                    if (marker.text.length !== 0) {
                        this.text.drawText3dWithDistance(
                            marker.text,
                            marker.position.x, marker.position.y, marker.position.z+ 1, 0.4, 0,
                            255, 255, 255, 175, false, true, 10);
                    }
                     
                    if (this.player.isAduty && marker.ownerName && marker.createdAtJson) {
                        this.text.drawText3dWithDistance(
                            `Erstellt von: ${marker.ownerName}\nErstellt um: ${this.date.getDate(marker.createdAtJson)}`,
                            marker.position.x, marker.position.y, marker.position.z + 0.5, 0.4, 0,
                            marker.color.red, marker.color.green, marker.color.blue, 255, false, true, 5);
                    }
                }
            }
        });
    ...
    public add(id: number, markerType: MarkerType, position: Vector3, direction: Vector3, rotation: Vector3,
               scale: Vector3, color: RGBAInterface, bobUpDown: boolean, text: string, ownerName: string, createdAtJson: string): void {
